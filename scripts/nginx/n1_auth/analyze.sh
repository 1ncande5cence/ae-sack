...
# full successful attack  (all the sub that breaks the oracle)

grep -a 'HTTP/1.1" 200' ./log/host.access.log | grep -v 'HTTP/1.1" 200 0' | awk '{print $1}' | sort -u > full_success_attack.log
