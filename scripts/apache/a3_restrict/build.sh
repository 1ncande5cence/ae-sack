...
cp $SACK/scripts/apache/a3_restrict/httpd.conf /usr/local/apache2/conf/
cp $SACK/scripts/apache/a3_restrict/httpd.conf /usr/local/apache2/conf.bak/
$SACK/AFL/afl-clang-fast-indirect-flip httpd.bc -o httpd.fuzz -L/target/httpd-restrict/srclib/apr/.libs -L/usr/local/openssl/lib -Wl,-rpath,/target/httpd-restrict/srclib/apr/.libs -Wl,-rpath,/usr/local/openssl/lib -lapr-2 -lpcre2-8 -luuid -lrt -lcrypt -lpthread -ldl -lexpat -lssl -lcrypto -lnghttp2 -lxml2 -llua5.1 -licuuc -llzma -licudata -lz -Wl,--export-dynamic

# -------------------- prepare tools and environments --------------------------

bash $SACK/scripts/apache/a3_restrict/copy_tools.sh $SACK .
...
# use corresponding analyze.sh from /ae-sack/scripts/{target_program}/{target_oracle} in the directory workdir /target/httpd-restirct/
