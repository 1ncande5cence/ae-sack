...
while True:
    try:
        response = requests.get(url, auth=HTTPBasicAuth("song", "wrong")) # local or valid-user-fail
        # response = requests.get(url, auth=HTTPBasicAuth("song", "secret")) # valid-user-pass
