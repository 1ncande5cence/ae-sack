...
url = "http://localhost/private/index.html"

output_file = "response_log.txt"

while True:
    try:
        # response = requests.get(url, params={"name": "admin password"}) # subgt
        response = requests.get(url, params={"name": "admin select password"}) # oracle
