...
time_t nullptr_start, nullptr_end;
...
EXP_ST u64 id_limit   = 0xFFFF;       /* the maximum ID for cond var  */
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           total_icalls,              /* Total indirect calls             */
           ...
           block_coverage_changed,    /* times when block coverage changed*/
           branch_coverage_changed,   /* times when branch coverage changed*/
           icall_changed_when_blc_changed,/* times icall changed when block coverage changed*/
           ipair_changed_when_brc_changed,/* times ipair changed when branch coverage changed*/
...
EXP_ST void write_performance_data(int icall_id, u64 flipped_target_address, int target_enter_times) {
  static int prev_icall_id;
  static int prev_target_address;

  if (prev_icall_id == icall_id &&
      prev_target_address == flipped_target_address) return;
  prev_icall_id = icall_id;
  prev_target_address = flipped_target_address;

  u8* fname = alloc_printf("%s/perf.txt", out_dir);
  FILE* f = fopen(fname, "a+");
  if (f==NULL) PFATAL("Unable to open '%s'", fname);

  struct address_entry* current = overall_icall_enter_times[icall_id];
  if (current == NULL) return;

  fprintf(f, "[%llu] icall: %d, total_enter_times: %d, numof_target: %d, target: 0x%llx (%llu), enter_times: %d\n",
             	    (get_cur_time() - start_time) / 1000,
		    icall_id, overall_icall_enter_times[icall_id],
		    overall_icall_numof_target[icall_id],
		    flipped_target_address, flipped_target_address, target_enter_times+1);
...
static inline u8 has_new_bits(u8* virgin_map, u32 dry_run_flag, u32 fuzzing_flag) {
...
#ifdef WORD_SIZE_64
        /* We construct the mapping and initialize the overall_icall map here, no need, it will be handled in the following codes in this function*/  
  ...
  while (fread(&tmp_id, size_in_bytes, 1, address_log) == 1 && fread(&tmp_addr, size_in_bytes, 1, address_log) == 1){
      ...
      if (!icall_search(cur_ovep_gt, tmp_addr)) {
        icall_violation = 1;
        ACTF("New icall/icall targets, id:address-%lld-%lld(0x%llx)",tmp_id, tmp_addr, tmp_addr);
...
static u8 run_target(char** argv, u32 timeout, u32 dry_run_flag, u32 icall_id, u32 target_id, u64 flipped_target_address, u32 null_ptr_mode_flag) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    *(u32 *)(tmp + 8) = icall_id;
    *(u32 *)(tmp + 12) = target_id;
    *(u64 *)(tmp + 16) = flipped_target_address;
    *(u32 *)(tmp + 24) = null_ptr_mode_flag;   // 1 -> null substitution mode  ,  0-> normal flip mode  

    if ((res = write(fsrv_ctl_fd, tmp, 28)) != 28) {

      if (stop_soon) return 0;
      RPFATAL(res, "Fail to write correctly from fork server");
    ...
    if ((res = read(fsrv_st_fd, &child_pid, 4)) != 4) {
      ...
      if (stop_soon) return 0;
      RPFATAL(res, "Fail to read correctly from fork server");
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    /*Ask the forkserver to dry_run a test case */
    fault = run_target(argv, use_tmout, 1 , NOT_TRIGGERED, NOT_TRIGGERED, NOT_TRIGGERED, 0);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault, u32 dry_run_flag, u32 fuzzing_flag) {
  ...
  if (1) {
    ...
    add_to_queue(fn, len, 0);  // just save the name, not content
...
static void maybe_update_plot_file(double bitmap_cvg, double eps, unsigned int block_triggered, double bb_ratio) {
  ...
  fprintf(plot_file, 
          ...
          block_coverage_changed, icall_changed_when_blc_changed, branch_coverage_changed, ipair_changed_when_brc_changed,
          block_triggered, bb_ratio, total_icalls, total_ipairs); /* ignore errors */
...
static void update_plot_file(double bitmap_cvg, double eps, unsigned int block_triggered, double block_coverage) {
  ...
  fprintf(plot_file, 
          ...
          unique_hangs, max_depth, eps, 
          block_coverage_changed, icall_changed_when_blc_changed, branch_coverage_changed, ipair_changed_when_brc_changed,
          block_triggered, block_coverage, total_icalls, total_ipairs); /* ignore errors */
  fflush(plot_file);
...
static void update_dry_run_file(double bitmap_cvg, double eps) {
  ...
  fprintf(dry_run_file, 
          ...
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          unique_hangs, max_depth, eps, total_icalls, total_ipairs); /* ignore errors */
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/dry_run_data", out_dir);
  if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
  ck_free(fn);
...
static void show_stats(void) {
  ...
  double t_byte_ratio, stab_ratio , bb_ratio;

  u64 cur_ms;
  u32 t_bytes, t_bits, bb_count;
  ...
  if (cur_ms - last_plot_ms > PLOT_UPDATE_SEC * 1000) {
    ...
    last_plot_ms = cur_ms;
    maybe_update_plot_file(t_byte_ratio, avg_exec, bb_count, bb_ratio);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len, u32 icall_id, u32 target_id, u32 fuzzing_flag, u64 flipped_target_address ,u32 null_ptr_mode_flag) {
...
static u8 fuzz_one(char** argv) {
  ...
  memset(overall_icall_enter_times, 0, MAP_SIZE * sizeof(u32));
  memset(overall_icall_numof_target, 0, MAP_SIZE * sizeof(u32));
  ...
  if (!dir) FATAL("Unable to open directory: %s", ADDRESS_LOG_SUB_GROUNDTRUTH);
  ...
  while ((entry = readdir(dir)) != NULL) {
	...
	strcmp(entry->d_name, "..") == 0)
     ...
     if (!address_s_log) PFATAL("Unable to load sub-gt '%s'", filepath);

     /* reduce time complexity */
     struct address_entry *cur_ovep_s = NULL;
     while (fread(&tmp_id_s, size_in_bytes_s, 1, address_s_log) == 1 && 
           fread(&tmp_addr_s, size_in_bytes_s, 1, address_s_log) == 1) {
         cur_ovep_s = subgt_load_flip[tmp_id_s];

         if (!icall_search(cur_ovep_s, tmp_addr_s)) {
             // ACTF("loading icall id %d", tmp_id_s);
             icall_append(&subgt_load_flip[tmp_id_s],tmp_addr_s, tmp_id_s);
         }
     }
  ...
  // loop and get the total number of targets
  for (s32 icall_id = 0; icall_id < MAP_SIZE; icall_id++) {
    current = subgt_load_flip[icall_id];
  ...
  u32 total_numof_triggered_icall = 0;
  u32 total_numof_triggered_target = 0;
  for (s32 icall_id = 0; icall_id < MAP_SIZE; icall_id++) {
    // if (icall_id == 111) continue;  // weird icall 33 will leftover a nginx process and disrupt the fuzzing
    current = subgt_load_flip[icall_id];
    if (current == NULL) continue;
    ...
    while (current != NULL) {
      ...
      while (true) {
          ...
          //   continue;
          // }
          //if (flipped_target_address == 6161440 || flipped_target_address == 3473107 || flipped_target_address == 4652176) break;
          
          // check if we have process "nginx: master process ./nginx.fuzz" 
          // if so, kill it before we do the next flipping
      ...
      if (target_id != 0) total_numof_triggered_target++;
      current = current->next;
      write_performance_data(icall_id, flipped_target_address, target_id);
    }    
  }
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    while ((qd_ent = readdir(qd))) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        fault = run_target(argv, exec_tmout,1, NOT_TRIGGERED, NOT_TRIGGERED, NOT_TRIGGERED,0);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "block_coverage_changed, icall_changed_when_blc_changed, branch_coverage_changed, ipair_changed_when_brc_changed,  "
                     "block_triggered, block_coverage, total_icalls, total_ipairs\n");
                     /* ignore errors */

  tmp = alloc_printf("%s/dry_run_data", out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  dry_run_file = fdopen(fd, "w");
  if (!dry_run_file) PFATAL("fdopen() failed");

  fprintf(dry_run_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     "pending_total, pending_favs, map_size, unique_crashes, "
                     "unique_hangs, max_depth, execs_per_sec, total_icalls, total_ipairs\n");
                     /* ignore errors */
}
...
void collect_dryrun() {
  ...
  memset(dryrun_load_flip, 0, MAP_SIZE * sizeof(u32));

  /* reduce time complexity */
  while (fread(&tmp_id, size_in_bytes, 1, address_log) == 1 && 
	fread(&tmp_addr, size_in_bytes, 1, address_log) == 1){
      dryrun_load_flip[tmp_id] = 1;
  }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QVD:a:c:")) > 0)
    ...
    switch (opt) {
      ...
      case 'M': { /* master sync ID */
          ...
          force_deterministic = 1;  //Skip deterministic stages?
      ...
      case 'm': { /* mem limit */
          ...
          if (!strcmp(optarg, "none")) {
            ...
            mem_limit = 0;  //Memory cap for child (MB)
            break;
  ...
  // output dry_run icalls and ipairs
  update_dry_run_file(0,0);
